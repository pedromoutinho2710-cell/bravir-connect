...
export default function NovoPedidoGestora() {
  ...
  // Busca pedido mínimo do representante selecionado
  useEffect(() => {
    if (!representanteId) { setPedidoMinimo(0); return; }
    supabase
      .from("profiles")
      .select("pedido_minimo")
      .eq("id", representanteId)
      .single()
      .then(({ data }) => setPedidoMinimo(data?.pedido_minimo ?? 0));
  }, [representanteId]);

  // Restaura pedido a partir de navegação com state (ex.: editar rascunho)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (ped.vendedor_id) setRepresentanteId(ped.vendedor_id);
    })();
  }, [hookLoading]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="sticky bottom-0 bg-background/95 backdrop-blur-sm border-t z-10 -mx-4 px-4 py-3 space-y-3">
        {/* Progresso — só exibe se o representante tem pedido mínimo configurado */}
        {pedidoMinimo > 0 && (
          <div className="space-y-1.5">
            <div className="flex justify-between text-sm">
              <span className="font-medium">Pedido mínimo ({representanteNome}): {formatBRL(pedidoMinimo)}</span>
      ...
      {/* Preview Dialog — agrupado por marca */}
          ...
          <div className="space-y-4 text-sm">
            <div>
              ...
              <div className="text-muted-foreground">
                CNPJ {formatCNPJ(cliente.cnpj)} • {cliente.cidade || "—"}/{cliente.uf || "—"}
              </div>
              <div className="text-muted-foreground">
              ...
              {representanteNome && (
                <div className="text-muted-foreground">
                  Representante: <strong>{representanteNome}</strong>
                </div>
              )}
            </div>
            {Object.entries(
